...
async function getUserFromToken(event: H3Event): Promise<User | null> {
  try {
    ...
    const botToken = import.meta.dev ? telegram.devBotToken : telegram.hubBotToken
    ...
    const userInDB = await db.telegram.findUserByTelegramIdAndBotId(telegramData.user.id.toString(), telegram.hubBotId)
    if (!userInDB?.user) {
      return null
    }
